...
CREATE table locationtable WITH (PARTITIONS=1, value_format='JSON') AS select ico, height, location, aircraft, registration, manufacturer, count(*) as events from location_and_details_stream WINDOW TUMBLING (size 10 second) group by ico, height, location, aircraft, registration, manufacturer;

-- 6. combined result, ident_stream join callsign
-- drop stream ident_callsign_stream;
create stream ident_callsign_stream as select i.ICO, c.operatorname, c.callsign, c.fromairport, c.toairport from ident_stream i left join callsign_details c on i.INDENTIFICATION = c.k_callsign;

-- drop table callsigntable;
CREATE table callsigntable WITH (PARTITIONS=1, value_format='JSON') AS select ICO, callsign, operatorname, fromairport, toairport, count(*) as events from ident_callsign_stream WINDOW TUMBLING (size 10 second) group by ICO, callsign, operatorname, fromairport, toairport;
